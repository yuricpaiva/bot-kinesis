...
- A tabela `cancelamentos` deve receber eventos onde `fiscalXmlCancel` estiver preenchido e tambem cancelamentos operacionais fortes sem cancelamento fiscal.
...
- Eventos com `stateId = 4`, `customProperties.VOID_AT` preenchido ou `customProperties.MANUAL_CANCELLATION` preenchido devem marcar `dw.vendas.cancelado = true`, mesmo quando `fiscalXmlCancel` estiver vazio. Nao usar `REMOTE_ORDER_STATUS = 5` sozinho como cancelamento confirmado.
...
Uma linha por venda cancelada. Considerar venda cancelada quando `fiscalXmlCancel` estiver preenchido ou quando houver cancelamento operacional forte (`stateId = 4`, `VOID_AT` ou `MANUAL_CANCELLATION`). Eventos `VOID_PAID_ORDER` com `fiscalXmlCancel` preenchido devem atualizar a venda existente como cancelada; eventos `VOID_CURRENT_ORDER` nao entram aqui, salvo se no futuro vierem acompanhados de cancelamento fiscal.
